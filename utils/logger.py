...
def setup_logging(args: Dict[str, Any]) -> logging.Logger:
    ...
    # Log initial info with relevant parameters only
    logger.info("Starting training with configuration:")
    _log_relevant_parameters(args, logger)
    
    return logger

def _log_relevant_parameters(args: Dict[str, Any], logger) -> None:
    """Log only parameters relevant to the current training method."""
    args_dict = vars(args)
    training_method = args_dict.get('training_method', 'freeze')
    
    # Define parameter groups
    lora_params = {'lora_r', 'lora_alpha', 'lora_dropout', 'lora_target_modules', 'feedforward_modules'}
    ses_adapter_params = {'structure_seq'}
    
    # Log all parameters except those not relevant to current method
    for key, value in args_dict.items():
        # Skip LoRA parameters if not using LoRA-based methods
        if key in lora_params and training_method not in ['lora', 'plm-lora', 'plm-qlora', 'plm-adalora', 'plm-dora']:
            continue
        # Skip SES-adapter parameters if not using SES-adapter
        if key in ses_adapter_params and training_method != 'ses-adapter':
            continue
